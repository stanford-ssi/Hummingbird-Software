...
void loop() {
  ...
  int index = 0;
  static int loop_counter = 0;  // Counter to track iterations
  if (rf95.available()) {
  ...
  // Every 5 loops, read PT_INPUT and transmit data
  if (loop_counter % 5 == 0) {
    int pt = analogRead(PT_INPUT); // Read pressure transducer value
    char message[32];
    snprintf(message, sizeof(message), "PT: %d", pt);
