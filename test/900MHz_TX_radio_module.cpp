...
void loop(){
    ...
    radio_packet.message = "Hello!";
    radio_packet.packetType = 0;
    radio_packet.packetLength = sizeof(radio_packet.message);

    
    while (true){
        Serial.println("Sending a message!");
        Serial.printf("Message being sent: %s", radio_packet.message);
        rf95._sendMessage(radio_packet.packetLength, &radio_packet);
